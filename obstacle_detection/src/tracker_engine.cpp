...
double TrackerEngine::dist(const geometry_msgs::msg::Point &a, const geometry_msgs::msg::Point &b)
{
...
std::vector<ObstacleTrack> TrackerEngine::update(const std::vector<BlobCluster> &clusters)
{
  // Update dt
  ...
  retired_ids_.clear();  // Reset retired list

  const size_t M = tracks_.size();
  const size_t N = clusters.size();

  // First cycle: birth all detections
  if (M == 0)
  {
    for (const auto &cluster : clusters)
    {
      ...
      t.predicted_next_position = cluster.centroid;  // Initial guess
  ...
  // Gating: accept matches within gate_distance_
...
void TrackerEngine::updatePredictedPositions(const std::unordered_map<int64_t, geometry_msgs::msg::Point> &predicted_t1)
{
  for (auto &t : tracks_)
  {
